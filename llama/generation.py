...
class Llama:
    ...
    def build(
        ...
        # 读取分词器模型
        tokenizer = Tokenizer(model_path=tokenizer_path)                # "llama-models/tokenizer.model"
        
        model_args.vocab_size = tokenizer.n_words
        torch.set_default_tensor_type(torch.cuda.HalfTensor)            # 设置默认精度类型为Half类型
        
        # 通过checkpoint初始化模型
        model = Transformer(model_args) 
        model.load_state_dict(checkpoint, strict=False)                 # 加载模型参数到Transformer中
        print(f"Loaded in {time.time() - start_time:.2f} seconds")      

        return Llama(model, tokenizer)                                  # 将Transformer模型和Tokenizer模型存储到LLAMA结构中
